"""
AWS Builder Center - Live Dynamic Dashboard (v9 - Optimized)
=============================================================
Fully dynamic Flask dashboard with fast single-pass scraping.
- Skips failing Chrome token step → straight to Selenium page scrape.
- Single Chrome instance captures API responses from network logs.
- Auto-scrape every 5 min + manual "Update Now" button.
...
# Import scraper bits we still need
...
def tag_competition(post):
    """Ensure is_competition flag is set."""
    if post.get("is_competition") is True:
        return post
    try:
        is_comp = is_competition_post(post)
    except Exception:
        is_comp = False
    if not is_comp:
        title = (post.get("title") or "").lower()
        is_comp = any(kw in title for kw in COMP_KW)
    post["is_competition"] = is_comp
    return post
    ...
    if os.path.exists(JSON_PATH):
        try:
            with open(JSON_PATH, "r", encoding="utf-8") as f:
                posts = json.load(f)
            posts = [tag_competition(p) for p in posts]
            mt = os.path.getmtime(JSON_PATH)
            return posts, datetime.fromtimestamp(mt).strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            pass
    return [], None
...
def fast_scrape():
    """
    Single-pass scrape: one Chrome instance → loads page → scrolls →
    captures API responses from network performance logs.
    Skips the token-capture step that always fails on Render.
    """
    ...
    try:
        ...
        # Visit the main page → this triggers feed API calls
        log("Loading builder.aws.com...")
        driver.get(BASE_URL)
        time.sleep(4)
        ...
        log("Extracting data from network logs...")
                        ...
                        try:
                            ...
                            for item in items:
                                ...
                                if uri:
                                    ...
                                    url_ = f"{BASE_URL}/content/{cid.split('/')[-1]}"
                                else:
                                    url_ = BASE_URL
        ...
        log(f"Captured {len(all_posts)} posts from network logs")

        # If we still missed some, try a second page load
        if len(all_posts) < 10:
            log("Trying /posts page too...")
            ...
            perf_logs2 = driver.get_log("performance")
            for entry in perf_logs2:
                try:
                    msg = json.loads(entry["message"])["message"]
                    if msg["method"] == "Network.responseReceived":
                        url = msg["params"]["response"]["url"]
                            ...
                            try:
                                ...
                                for item in data.get("feedContents", []):
                                    ...
                                    post = {
                                        ...
                                        "last_published_at": format_timestamp(item.get("lastPublishedAt")),
                                        "uri": uri,
                                        "url": f"{BASE_URL}{uri}" if uri else BASE_URL,
                                        "status": item.get("status", ""),
                                        "locale": item.get("locale", ""),
                                        "author_alias": (item.get("author") or {}).get("alias", "N/A"),
                                        "author_name": (item.get("author") or {}).get("preferredName", "N/A"),
                    ...
                    continue
            log(f"After /posts: {len(all_posts)} total posts")
...
def run_scraper():
    ...
    try:
        raw_posts = fast_scrape()

        if raw_posts:
            sorted_posts = sorted(raw_posts, key=lambda x: x.get("likes_count", 0), reverse=True)
            seen = set()
            unique = []
            ...
            log(f"Done! {len(unique)} posts ({comp_n} comp) in {elapsed}s. Top: {unique[0]['likes_count']} likes")
        else:
            with data_lock:
                scrape_data["error"] = "No posts found. Site may be temporarily unavailable."
            log("Failed: 0 posts returned")

    except Exception as e:
        with data_lock:
            scrape_data["error"] = f"{type(e).__name__}: {str(e)}"
        log(f"Error: {e}")
        traceback.print_exc()
    finally:
        with data_lock:
...
def api_data():
    ...
    for idx, p in enumerate(posts, 1):
        ...
        is_hl = (hl and pid and pid in hl) or \
                (hl and puri and puri in hl) or \
...
<head>
    ...
    <style>
        :root{--bg:#050a14;--bg2:#0c1324;--cd:#111827;--cd2:#1a2236;--a:#ff9900;--a2:#ffb84d;--gw:rgba(255,153,0,.3);--g:#10b981;--b:#3b82f6;--r:#ef4444;--t:#f3f4f6;--t2:#9ca3af;--t3:#6b7280;--bd:#1f2937;--rad:14px}
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .ubtn:hover{transform:translateY(-1px);box-shadow:0 6px 24px var(--gw)}
        ...
        .proj{background:linear-gradient(135deg,var(--cd2),var(--cd));border:1px solid var(--bd);border-radius:var(--rad);padding:14px 16px;margin-bottom:12px;display:flex;align-items:center;gap:14px}
        .proj-i{font-size:26px}
        .proj-b{flex:1;min-width:0}
        ...
        .st{background:var(--cd);border:1px solid var(--bd);padding:11px;border-radius:var(--rad);text-align:center}
        ...
        .hl{background:linear-gradient(145deg,#1a263d,#0f172a);border:2px solid var(--a);border-radius:var(--rad);padding:16px;margin-bottom:12px;box-shadow:0 0 35px var(--gw);position:relative;overflow:hidden}
        ...
        .hl-bg{background:var(--a);color:#000;padding:3px 10px;font-size:9px;font-weight:800;border-radius:6px;text-transform:uppercase;letter-spacing:.5px}
        ...
        .ctx-h{font-size:9px;font-weight:800;color:var(--a);text-transform:uppercase;margin-bottom:5px;letter-spacing:.5px}
        ...
        .ctx-r span:last-child{font-weight:800;color:var(--a);font-size:11px}
        ...
        .fb{background:var(--cd);border:1px solid var(--bd);color:var(--t2);padding:6px 12px;border-radius:9px;font-size:11px;font-weight:700;cursor:pointer;transition:all .15s;font-family:'Outfit'}
        .fb.on{background:var(--a);color:#000;border-color:var(--a)}

        .ri{background:var(--cd);border:1px solid var(--bd);border-radius:var(--rad);padding:11px 13px;display:flex;align-items:center;gap:11px;margin-bottom:6px;transition:border-color .2s}
        ...
        .lp{background:var(--bg2);border:1px solid var(--bd);border-radius:8px;padding:6px 8px;margin-top:4px;font-size:9px;color:var(--t3);font-family:monospace;max-height:100px;overflow-y:auto;display:none}
        .lp.sh{display:block}
    </style>
</head>
<body>
<div class="mesh"></div>
<div id="ov"><div class="ld"></div><div style="font-size:12px;font-weight:700;color:var(--t2)">Loading Dashboard...</div></div>
<div class="w">
    <div class="hdr">
        <div>
            ...
            <div class="hm">
                <div class="dot"></div>
                <span class="mt" id="ut">Starting up...</span>
                <span class="mt" id="ns"></span>
            </div>
        </div>
        <button class="ubtn" id="ub" onclick="doRefresh()">
            <span id="utx">&#x21bb; Update Now</span>
        </button>
    </div>
    <div class="sb" id="sb" style="display:none"></div>
    <div class="proj">
        <div class="proj-i">🏥</div>
        <div class="proj-b">
            <div class="proj-n">AIdeas: Transforming Healthcare into AI-Powered Wellness Companion</div>
            <div class="proj-d">AI-powered wellness platform leveraging AWS services for proactive, personalized healthcare.</div>
            <div class="proj-a">By <strong>Md. Shafayat Sadat Saad</strong> (@saad30) — AWS Builder Competition 2026</div>
        </div>
    </div>
    <div class="sts">
    ...
    <div id="hlBox"></div>
    <div class="flt">
        <button class="fb on" id="fA" onclick="setF('all')">All Posts</button>
        <button class="fb" id="fC" onclick="setF('comp')">Competition</button>
    </div>
    <div id="list"></div>
...
async function loadData(){
    try{const r=await fetch('/api/data');D=await r.json();document.getElementById('ov').style.display='none';render()}catch(e){console.error(e)}
}
...
function render(){
    ...
    if(is_scraping){sb.style.display='flex';sb.className='sb busy';sb.innerHTML='<div class="sp"></div> Scraping builder.aws.com...'}
    ...
    if(hp&&hr){
        ...
        let cx='';
        if(nb&&nb.length>0){
            cx=`<div class="ctx"><div class="ctx-h">Rank Neighborhood</div>${nb.map(n=>`<div class="ctx-r"><span>#${n.rank} ${(n.title||'').length>28?(n.title||'').substring(0,25)+'...':n.title||''}</span><span>${n.likes_count} likes</span></div>`).join('')}${lc>0?`<div class="clmb">🚀 ${lc} more like${lc>1?'s':''} to reach #${hr-1}</div>`:''}</div>`;
        }
...
function renderList(){
    ...
    if(items.length===0){document.getElementById('list').innerHTML='<div style="text-align:center;padding:20px;color:var(--t3);font-size:12px">No posts found.</div>';return}
    document.getElementById('list').innerHTML=items.map(p=>{
        ...
        return `<div class="ri ${me?'mine':''}"><div class="rn" ${r<=3?'style="color:#fff;font-size:16px"':''}>${m}</div><div class="rb"><a href="${p.url||'#'}" target="_blank" class="rt">${p.title||'Untitled'}</a><div class="rm"><span>${au}</span>${p.is_competition?'<span class="rc">Competition</span>':''}</div></div><div class="rl"><span class="rv">${p.likes_count}</span><span class="rll">likes</span></div></div>`;
    }).join('');
}
...
async function doRefresh(){
    ...
    try{
        ...
        const iv=setInterval(async()=>{
            ...
            }catch(e){if(n>120){clearInterval(iv);b.disabled=false;t.innerHTML='&#x21bb; Update Now'}}
        },2000);
